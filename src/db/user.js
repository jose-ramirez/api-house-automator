...
schema.static('resetToken', function (email, token, callback) {
    ...
    that.findOne({email: email}, function (err, user) {
        ...
        if (user.isActivated) {
            var msg = 'The user has activated, no active again!';
        ...
        that.findOneAndUpdate({email: email}, {
            ...
            var msg = 'Reset token success!';
            callback(null, user.toObject({transform: transform}), msg);
        });
    });
});
...
schema.static('active', function (email, token, callback) {
    ...
    that.findOne({email: email}, function (err, user) {
        ...
        if (user.validExpire && user.validExpire < Date.now()) {
            var errMsg = 'Activation time has expired, please re-activate!';
            return callback(null, null, errMsg);
        }
